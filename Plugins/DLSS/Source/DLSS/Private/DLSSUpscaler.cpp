...
static TAutoConsoleVariable<int32> CVarNGXDLSSBiasCurrentColorMaskMode(
	TEXT("r.NGX.DLSS.BiasCurrentColorMaskMode"),
	0,
	TEXT("Set the mode for biasing of the current frame.\n")
	TEXT("0: Use internal engine mask (default).\n")
	TEXT("1: Use custom stencil.\n"),
	ECVF_RenderThreadSafe
);
...
) const
{
...

	{
		...
		bool bBiasUsingCustomStencil = CVarNGXDLSSBiasCurrentColorMaskMode.GetValueOnRenderThread() != 0;
		if (bBiasUsingCustomStencil)
		{
		...
		{
			BiasCurrentColorTexture = PassInputs.SceneColorMask.Texture;
		}
